...
void main() {
  group('Mock Authentication', () {
    ...
    test('Should be able to initialize in less than 2 seconds', () async {
      await provider.initialize();
      expect(provider.isInitialized, bool);
    },
    timeout: const Timeout(Duration(seconds: 2)));

    test('Create user should delegate to logIn function', () async {
      final badEmailUser = await provider.createUser(email: 'foo@bar.com', password: 'anyPassword');
      expect(badEmailUser, throwsA(const TypeMatcher<UserNotFoundAuthException>()));
      final badPasswordUser = await provider.createUser(email: 'someone@bar.com', password: 'foobar');
